# Feature Specification: Midnight Proof System Verifier (Spec 006)

**Feature Branch**: `006-midnight-proof-verifier`  
**Created**: 2026-02-24  
**Status**: Draft  
...
- Performance evaluation for this spec uses the same workload profile and measurement method defined by Spec 002 SC-004 (fixed corpus up to 10KB, offline execution, warmed process, p95 computed over at least 100 measured validations).

## Clarifications

### Session 2026-02-24

- Q: Should Spec 006 inherit the exact performance workload and p95 target from Spec 002, or define a standalone target? → A: Inherit Spec 002 exact workload and p95 target verbatim; keep Midnight verifier budget cap at <= 60% of that p95 budget.
...
- **SC-004**: Using the exact Spec 002 SC-004 workload and method, validation completes in under 1 second for 95% of envelopes up to 10KB when offline, and Midnight verification consumes no more than 60% of that p95 budget.
